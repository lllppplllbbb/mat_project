...
def setup_training_loop_kwargs(
    ...
    print(f"Data_val path: {data_val}")
    if not os.path.exists(data):
        raise UserError(f"Training data path does not exist: {data}")
    if not os.path.exists(data_val):
        raise UserError(f"Validation data path does not exist: {data_val}")
    seg_train_dir = os.path.join(os.path.dirname(data), 'segmentations/train')
    seg_test_dir = os.path.join(os.path.dirname(data_val), 'segmentations/test')
    if not os.path.exists(seg_train_dir):
        raise UserError(f"Segmentation train path does not exist: {seg_train_dir}")
    if not os.path.exists(seg_test_dir):
        raise UserError(f"Segmentation test path does not exist: {seg_test_dir}")
    ...
    args.val_set_kwargs = dnnlib.EasyDict(class_name=dataloader, path=data_val,
                                          use_labels=True, max_size=None, xflip=False,
                                          seg_dir=seg_test_dir)
    args.data_loader_kwargs = dnnlib.EasyDict(pin_memory=True, num_workers=3, prefetch_factor=2)

    try:
        print(f"Loading training set: {args.training_set_kwargs}")
        ...
        print(f"Loading validation set: {args.val_set_kwargs}")
        ...
        print(f"Dataset loading error: {err}")
    ...
    if fp32:
        desc += '-fp32'
    print(f"G_kwargs after fp32: {args.G_kwargs}")
    print(f"D_kwargs after fp32: {args.D_kwargs}")
...
def main(ctx, outdir, dry_run, **config_kwargs):
    """Train a GAN using the techniques described in the paper
    "Training Generative Adversarial Networks with Limited Data"."""
    print(f"Main: Received kwargs: {config_kwargs}")
