...
for i in range(5):
    sample = dataset[i]
    img_fname = dataset._image_fnames[dataset._raw_idx[i]]
    seg_fname = dataset.segs[dataset._raw_idx[i]]
    print(f"Image: {img_fname}, Seg: {seg_fname}")
sample = dataset[0]
print(len(sample), [x.shape for x in sample], sample[2].min(), sample[2].max(), sample[3].shape)

from datasets.dataset_512_val import ImageFolderMaskDataset as ValDataset
val_dataset = ValDataset(
    path='F:/MAT_project/MAT/data/test_images',
    seg_dir='F:/MAT_project/MAT/data/segmentations/test',
    resolution=512
)
for i in range(5):
    val_sample = val_dataset[i]
    img_fname = val_dataset._image_fnames[val_dataset._raw_idx[i]]
    seg_fname = val_dataset.segs[val_dataset._raw_idx[i]]
    print(f"Val Image: {img_fname}, Val Seg: {seg_fname}")
val_sample = val_dataset[0]
print(len(val_sample), [x.shape for x in val_sample], val_sample[2].min(), val_sample[2].max(), val_sample[3].shape)